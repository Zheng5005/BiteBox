...
const CreateCookbookModal: React.FC<CreateCookbookModalProps> = ({ onClose, onCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await createCookbook(name.trim(), description.trim(), isPublic);
      onCreated();
    } catch {
      setError('Failed to create cookbook.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold">Create Cookbook</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="cookbook-name" className="block text-sm font-medium text-gray-900">
              Name
            </label>
            <input
              id="cookbook-name"
          ...
          <div>
            <label htmlFor="cookbook-description" className="block text-sm font-medium text-gray-900">
              Description
            </label>
            <textarea
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="cookbook-public" className="text-sm text-gray-700">
